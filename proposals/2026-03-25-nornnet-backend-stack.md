...
> ### Det anbefales at etablere en komponerbar, åben backend-stack baseret på containeriserede services, hvor hvert komponent er bytteligt og infrastrukturen defineres som kode.

Alle services køres som containere – enten via **Podman med systemd-quadlets** eller på en eksisterende **Kubernetes-platform** (f.eks. K3s). Valg af kørselsplatform afhænger af det eksisterende driftsmiljø.
...
- **Git-repositorier** for alle bootc-image-definitions, konfigurationer og pipelines
...
Forgejo understøtter SAML/OIDC integration, hvilket gør det muligt at koble til en ekstern IdP.
...
Keycloak fungerer som den primære IdP for backend-services. Forgejo og andre services autentificerer brugere via OIDC mod Keycloak. Dette gør det muligt at styre adgange ét sted og understøtte SSO på tværs af services.
...
Alle backend-services placeres bag en reverse proxy, der håndterer TLS-certifikater og trafik-routing.
- **Caddy:** Enkel konfiguration, automatiske HTTPS-certifikater via Let's Encrypt.
- **Traefik:** Bedre egnet til containeriserede miljøer med automatisk service-opdagelse.
...
> **I produktion vælger den enkelte implementering selv sit overvågningsværktøj.** Perses anvendes her til demo/PoC. Andre muligheder inkluderer [SigNoz](https://signoz.io/) (Apache 2.0), [Ace Observability](https://aceobservability.com/) eller lignende OSI-licenserede alternativer.

### **Secret Management (OpenBao / SOPS)**

> Sikker opbevaring af credentials, certifikater og hemmeligheder.

CI/CD-pipelines har brug for adgang til credentials (registry tokens, signing keys, etc.).
- **[OpenBao](https://openbao.org/):** Linux Foundation-fork af Vault med åben licens (MPL 2.0). Fuldt featured secret management med dynamiske secrets.
...
Når en Pull Request merges i et image-repositorie, trigger Forgejo Actions en pipeline der:
...
Telemetry data sendes fra enhederne via pull-model (enhederne pusher data til et endpoint). Data lagres i en tidsrækkedatabase (f.eks. InfluxDB, Prometheus, eller PostgreSQL afhængigt af datatyper).
...
| **Egnethed til PoC** | Ideel til mindre udrulninger | Bedre egnet til større miljøer |
| **Orkestrering** | systemd styrer alt | K8s styrer alt |
| **Højest tilgængelighed** | Kræver manuel opsætning | Indbygget med K8s |
...
### 🔓 Fuldt åben stack
> **Ingen proprietære afhængigheder.** Alle komponenter er open source. Hvert komponent kan byttes ud uden at ændre den overordnede arkitektur.

### 📦 Komponerbar arkitektur
> **Byt komponenter efter behov.** Er Caddy for simpel? Skift til Traefik. Har vi allerede OpenBao? Brug den i stedet for SOPS. Arkitekturen er designet til valgfrihed.

### 🔄 Infrastruktur som kode
> **Hele backenden defineres i Git.** Compose-filer, quadlet-definitioner, CI/CD-pipelines – alt er versioneret, reviewet og auditable via Pull Requests.

### 📏 12-factor compliance
> **Hver service er konfigureret via miljøvariabler, er stateless hvor muligt, og kan skaleres uafhængigt.** Dette letter drift, backup og genetablering.
...
- **Kompenserbar arkitektur:** Hvert komponent kan byttes uden systemændring
...
- **Kompleksitet af mange komponenter**
  _Afbødes gennem standardiserede Compose-filer og dokumentation. Mange komponenter er "set and forget" i drift._

- **Kompetencebehov**
  _Driftsteamet skal mestre containerisering, GitOps og de enkelte services. Afbødes gennem fælles træning._

- **PoC til skalaering**
  _Podman + Quadlets kan udgøre en flaskehals ved større udrulninger. Afbødes ved at designe images til at være platform-uafhængige._
